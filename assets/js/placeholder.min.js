...
(function() {
	...
	var Placeholder = function (el) {
		...
		this.placeholderImage = this.el.getAttribute('data-placeholder-image') === undefined ? '' : this.el.getAttribute('data-placeholder-image');
		this._init();
	};
	...
	Placeholder.prototype = {
		...
		_init: function () {
			...
			if (me.password && this.placeholderImage) {
				//me.el.style.backgroundImage = this.placeholderImage;
				me.el.style.backgroundImage = "url('" + me.placeholderImage + "')";
				me.el.style.backgroundRepeat = 'no-repeat';
			} else if (me.password) {
		...
		_bindEvents: function () {
			...
			if (me.password && this.placeholderImage) {
				...
				me.el.attachEvent('onchange', function () {
					if (me.el.value === '' || me.el.value === me.placeholder()) {
						me.el.style.backgroundImage = "url('" + me.placeholderImage + "')";
					}
				});
	...
	var attrHooks = {
		set: function (el, val, name) {
			if (name != 'placeholder') { return $(el); }
			var oldPlaceholder = el.getAttribute('placeholder') || '';
			el.setAttribute('placeholder', val);
			if (el.type === 'password') {
				$(el).siblings('.ui_fake').val(val);
			} else {
				if (!el.value || el.value === oldPlaceholder) {
					el.value = val || '';
				}
			}
	...
	setInterval(function () {
		...
		if (!bindjQueryHooks && window.$) {
			$.propHooks.value = $.valHooks.input = $.valHooks.textarea = hooks;
			$.attrHooks.placeholder = attrHooks;
